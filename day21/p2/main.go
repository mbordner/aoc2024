...
func main() {
	...
	for i, line := range lines {
		...
		length := getPathSeqMinLength(line, 25)
		fmt.Println(length)
		sum += length * getNumVal(line)
	}
...
var (
	...
	memDepthSequenceMinLength = make(map[DepthSequence]int)
)

func getPathSeqMinLength(sequence string, depth int) int {
	numPadDirections := getSequencesForSequence(numPadGraph, sequence)
	minLength := math.MaxInt64
	for _, seq := range numPadDirections {
		l := getPathSeqLength(seq, depth)
		if l < minLength {
			minLength = l
		}
	}
...
func getPathSeqLength(sequence string, depth int) int {
	if l, e := memDepthSequenceMinLength[DepthSequence{sequence, depth}]; e {
		return l
	}

	dirSequence := "A" + sequence
	length := 0

	if depth == 1 {
		for i := 0; i < len(dirSequence)-1; i++ {
			sequences := getPathsForSequence(dirPadGraph, dirSequence[i:i+2])
	...
	for i := 0; i < len(dirSequence)-1; i++ {
		sequences := getPathsForSequence(dirPadGraph, dirSequence[i:i+2])
		seqMinLength := math.MaxInt64
		for _, seq := range sequences {
			seqLength := getPathSeqLength(seq, depth-1)
	...
	memDepthSequenceMinLength[DepthSequence{sequence, depth}] = length
	return length
}
